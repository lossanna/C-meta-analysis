---
title: "Non-native (exotic) single moderator models"
author: "Lia Ossanna"
...
# Introduction
- This is the analysis of the summary effect and effect of a single moderator on non-native (exotic) plant abundance.
...
- Note that the word "exotic" is used to refer to the non-native dataset and models in the code.
...
- Analysis includes a model run with all the data, and a model run with outliers removed (see "Non-native (exotic) outliers for models" document for details; `code_nonnative-outliers.html`).
...
M <- ex.mv[["b"]] # summary treatment effect
SD.PI <- predict(ex.mv)$se * sqrt(ex.mv[["k"]]) # SD of prediction interval, calculated from SE

t <- as.numeric((D - M) / (SD.PI))

pt(t, ex.mv[["k"]] - 1) # left-tail cumulative t-distribution with k-1 df
```
